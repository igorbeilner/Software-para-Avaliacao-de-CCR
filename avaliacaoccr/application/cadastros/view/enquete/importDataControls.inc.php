<?php
	switch($_GET['acao']){
		...
		case 'gravar_enquete_importada':	
			...
			//echo "qtd: ".$_POST['enqimp_qtd_perg_imp']."<br />";
			//echo "vetor: ".$_POST['enqimp_perg_alter']."<br />";
			...
			for ($i = 0; $i < $total; $i++){
				//echo "<br/>i = ".$i;
				if (isset($_POST['enqimp_per_desc_'.$i.'escala'])){
					//echo $_POST['enqimp_per_desc_'.$i.'escala'];
					$array_import['per_cod'] = $_POST['escala_per_cod'.$i];
					$array_import['enq_cod'] = $enq_cod;
					$data->add($array_import);
				}

				if (isset($_POST['enqimp_per_desc_'.$i.'texto'])){
					//echo $_POST['enqimp_per_desc_'.$i.'texto'];
					$array_import_text['per_cod'] = $_POST['texto_per_cod'.$i];
					$array_import_text['enq_cod'] = $enq_cod;
...
<h2 style="margin-left: 20%;">ENQUETE GRAVADA COM SUCESSO!</h2>